...
int av_write_frame(AVFormatContext *s, AVPacket *pkt)
{
   ...
   int ret;
    av_log(s, AV_LOG_DEBUG, "writeFrame 1");
    ret = prepare_input_packet(s, pkt);
    av_log(s, AV_LOG_DEBUG, "writeFrame 2");
    if (ret < 0)
        return ret;
    av_log(s, AV_LOG_DEBUG, "writeFrame 3");	
    if (!pkt) {
	av_log(s, AV_LOG_DEBUG, "writeFrame 4");    
        if (s->oformat->flags & AVFMT_ALLOW_FLUSH) {
	    av_log(s, AV_LOG_DEBUG, "writeFrame 5");	
            ret = s->oformat->write_packet(s, NULL);
            flush_if_needed(s);
	    av_log(s, AV_LOG_DEBUG, "writeFrame 6");
            if (ret >= 0 && s->pb && s->pb->error < 0)
                ret = s->pb->error;
	    av_log(s, AV_LOG_DEBUG, "writeFrame 7");
            return ret;
        }
        return 1;
    }
    av_log(s, AV_LOG_DEBUG, "writeFrame 8");	
    ret = do_packet_auto_bsf(s, pkt);
    if (ret <= 0)
        return ret;
av_log(s, AV_LOG_DEBUG, "writeFrame 9");
#if FF_API_COMPUTE_PKT_FIELDS2 && FF_API_LAVF_AVCTX
    ret = compute_muxer_pkt_fields(s, s->streams[pkt->stream_index], pkt);
    	av_log(s, AV_LOG_DEBUG, "writeFrame 10");
    if (ret < 0 && !(s->oformat->flags & AVFMT_NOTIMESTAMPS))
        return ret;
#endif
av_log(s, AV_LOG_DEBUG, "writeFrame 11");
    ret = write_packet(s, pkt);
    if (ret >= 0 && s->pb && s->pb->error < 0)
        ret = s->pb->error;
av_log(s, AV_LOG_DEBUG, "writeFrame 12");
    if (ret >= 0)
        s->streams[pkt->stream_index]->nb_frames++;
   av_log(s, AV_LOG_DEBUG, "writeFrame 13");
    return ret;
}
...
{
    int ret, flush = 0;
av_log(s, AV_LOG_DEBUG, "av_writeFrame 1");
    ret = prepare_input_packet(s, pkt);
    if (ret < 0)
        goto fail;
   av_log(s, AV_LOG_DEBUG, "av_writeFrame 2");
    if (pkt) {
        AVStream *st = s->streams[pkt->stream_index];
	av_log(s, AV_LOG_DEBUG, "av_writeFrame 3");
        ret = do_packet_auto_bsf(s, pkt);
        if (ret == 0)
            return 0;
        else if (ret < 0)
            goto fail;
    av_log(s, AV_LOG_DEBUG, "av_writeFrame 4");
        if (s->debug & FF_FDEBUG_TS)
            av_log(s, AV_LOG_DEBUG, "av_interleaved_write_frame size:%d dts:%s pts:%s\n",
                pkt->size, av_ts2str(pkt->dts), av_ts2str(pkt->pts));
av_log(s, AV_LOG_DEBUG, "av_writeFrame 5");
#if FF_API_COMPUTE_PKT_FIELDS2 && FF_API_LAVF_AVCTX
        if ((ret = compute_muxer_pkt_fields(s, st, pkt)) < 0 && !(s->oformat->flags & AVFMT_NOTIMESTAMPS))
            goto fail;
#endif
av_log(s, AV_LOG_DEBUG, "av_writeFrame 6");
...
av_log(s, AV_LOG_DEBUG, "av_writeFrame 7");
    for (;; ) {
        AVPacket opkt;
        int ret = interleave_packet(s, &opkt, pkt, flush);
        if (pkt) {
		av_log(s, AV_LOG_DEBUG, "av_writeFrame 8");
        ...
        if (ret <= 0) //FIXME cleanup needed for ret<0 ?
            return ret;
av_log(s, AV_LOG_DEBUG, "av_writeFrame 9");
        ret = write_packet(s, &opkt);
        if (ret >= 0)
            s->streams[opkt.stream_index]->nb_frames++;
av_log(s, AV_LOG_DEBUG, "av_writeFrame 10");
